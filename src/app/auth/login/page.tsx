...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Sync local storage for context
            localStorage.setItem('user', JSON.stringify(data.user));

            router.push('/');
            setTimeout(() => window.location.reload(), 100); // Trigger reload to update context/navbar
        } catch (err: any) {
    ...
    return (
        ...
        .auth-success { background: #d1fae5; color: #065f46; padding: 12px; border-radius: 12px; font-size: 14px; text-align: center; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        .btn-auth { padding: 18px; background: var(--fg); color: white; border: none; border-radius: 16px; font-weight: 800; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; transition: all 0.3s; margin-top: 10px; }
        .btn-auth:hover { background: var(--primary); transform: translateY(-2px); box-shadow: 0 10px 25px var(--primary-light); }
